# 251Bet Printer Service / PrinterBridge

Local Node print bridge for cashier thermal receipts. Receives pre-encoded ESC/POS bytes from the admin cashier UI (`https://admin.251bet.net`) and writes them to the **POS80** Windows print queue via RAW spooler (PowerShell `WritePrinter` fallback).

Ticket APIs are served by **`https://api.251bet.net`** — the bridge only handles localhost printing; it does not proxy API traffic.
...
- CORS allows localhost and `https://admin.251bet.net` (override via `CASHIER_ORIGINS` env)
...
The admin app at **https://admin.251bet.net** posts to `http://localhost:3005` (auto-probes 3005–3010) with the API key header. Production build env:
