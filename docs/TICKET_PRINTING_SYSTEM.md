...
3. Installer copies to `C:\251Bet\PrinterBridge\`, writes `config.json`, adds Startup shortcut, starts exe.
...
public class RawPrinterHelper {
    ...
    public static int SendBytesToPrinter(string szPrinterName, byte[] bytes) {
        ...
        if (!OpenPrinter(szPrinterName, out hPrinter, IntPtr.Zero))
            return -1;
        try {
            var di = new DOCINFOW { pDocName = "251Bet Ticket", pDataType = "RAW" };
...
await esbuild.build({
  ...
  external: ["printer"],
  banner: {
    js: "/* 251Bet PrinterBridge bundled server */",
  },
});
