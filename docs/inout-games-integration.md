# InOut Games Integration — 251Bet

Complete reference for the **seamless-wallet** integration with [InOut Games](https://inout.games). 251Bet holds player balances; InOut calls our webhook for `init`, `bet`, `withdraw`, and `rollback`. Games launch in an iframe via InOut’s launch URL.

**Production callback URL:** `https://api.251bet.net/api/integrations/inout/webhook`  
**Currency:** ETB (Ethiopian Birr)  
**Brand:** 251Bet (InOut operator alias remains `michotbet-prod` until InOut reissues)  

> This document covers the **production** backend (`api.251bet.net`). We use one backend for both InOut certification and live play — no separate staging stack on the VPS.
...
| Topic | Choice |
|--------|--------|
| Wallet model | Seamless — 251Bet debits/credits ETB on webhook |
...
Maps InOut `authToken` → 251Bet user for webhooks.
...
curl -s -o /dev/null -w "%{http_code}\n" \
  -X POST https://api.251bet.net/api/integrations/inout/webhook
```
...
```
Webhook:     POST https://api.251bet.net/api/integrations/inout/webhook
Player lobby: https://251bet.net/casino
