...
2. **nginx**: Example site `deploy/nginx/api.251bet.net.conf` proxies **`api.251bet.net`** → `127.0.0.1:3001`, with a **`/nginx-health`** route answered by nginx (not Node).
...
- A domain/subdomain pointing at the server (example: **`api.251bet.net`** → server IPv4 A record).
...
```bash
sudo cp /path/to/your/repo/deploy/nginx/api.251bet.net.conf /etc/nginx/sites-available/api.251bet.net.conf
sudo ln -sf /etc/nginx/sites-available/api.251bet.net.conf /etc/nginx/sites-enabled/
...
sudo certbot --nginx -d api.251bet.net
```
...
Point your API base URL to **`https://api.251bet.net`** (`VITE_*` or equivalent). The backend historically used permissive CORS (`origin: "*"`); tighten for production when you pin allowed origins.
...
```bash
curl -sS https://api.251bet.net/health
```

HTTP only (before TLS):

```bash
curl -sS http://api.251bet.net/health
```

### 9.2 nginx only (does not hit Node)

```bash
curl -sS https://api.251bet.net/nginx-health
```

Expect **`ok`** (or whatever your `api.251bet.net.conf` returns).  
...
```bash
curl -sS -o /dev/null -w "%{http_code}\n" https://api.251bet.net/api/football/fixtures/today
```
...
| `deploy/nginx/api.251bet.net.conf` | Example nginx reverse proxy |
