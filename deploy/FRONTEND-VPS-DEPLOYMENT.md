...
| **API**          | HTTPS origin like `https://api.251bet.net` — injected at **build time**  |
...
- A hostname for the site (examples: **`https://251bet.net`**, **`https://bet.your-domain.com`**).
...
VITE_API_URL=https://api.251bet.net
...
```bash
sudo mkdir -p /var/www/251bet-frontend
# from your dev machine or CI:
rsync -avz --delete frontend/dist/ user@your-vps:/var/www/251bet-frontend/
```
...
sudo rsync -av --delete dist/ /var/www/251bet-frontend/
```
...
Example file in the repo: **`deploy/nginx/251bet-frontend.example.conf`**.

Install (replace domain and root path):

```bash
sudo cp /path/to/repo/deploy/nginx/251bet-frontend.example.conf /etc/nginx/sites-available/251bet-frontend.conf
sudo nano /etc/nginx/sites-available/251bet-frontend.conf   # server_name, root
sudo ln -sf /etc/nginx/sites-available/251bet-frontend.conf /etc/nginx/sites-enabled/
...
| `deploy/nginx/251bet-frontend.example.conf` | nginx template                   |
