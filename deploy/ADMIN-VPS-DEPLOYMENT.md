...
| **`API_URL`**    | Must point at the same backend as production (e.g. `https://api.251bet.net/api`) — baked in at **build time** |
...
```js
export const API_URL = "https://api.251bet.net/api";
```
...
```bash
sudo mkdir -p /var/www/251bet-admin
rsync -avz --delete admin/dist/ user@your-vps:/var/www/251bet-admin/
```
...
sudo rsync -av --delete dist/ /var/www/251bet-admin/
```
...
Example:** `deploy/nginx/251bet-admin.example.conf`**

Install:

```bash
sudo cp /path/to/repo/deploy/nginx/251bet-admin.example.conf /etc/nginx/sites-available/251bet-admin.conf
sudo nano /etc/nginx/sites-available/251bet-admin.conf
sudo ln -sf /etc/nginx/sites-available/251bet-admin.conf /etc/nginx/sites-enabled/
...
| `deploy/nginx/251bet-admin.example.conf` | nginx template                             |
