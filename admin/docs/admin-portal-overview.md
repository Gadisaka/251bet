# 251Bet admin portal — scope and pages

This document describes what **staff** (non-player) users do in the admin portal, how that maps to the **251Bet** platform (shops, cashiers, betting, wallets), and which **pages** the React admin app should provide. It is derived from `backend/docs/database-schema.md`, `backend/lib/permissions.js`, mounted API routes in `backend/index.js`, and the current admin shell in `admin/src`.
